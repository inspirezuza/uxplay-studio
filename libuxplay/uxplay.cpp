...
static void emit_host_event(uxplay_event_type type,
                            ...
                            const char *device_model = NULL,
                            const char *device_id = NULL,
                            const char *message = NULL,
                            int width = 0,
                            int height = 0) {
    if (!host_event_callback) {
        return;
    }
    uxplay_event event = {
...
extern "C" int uxplay_start_recording(const char *directory) {
    ...
    if (!recording_test_mode) {
        ...
        mux_to_file = true;
        mux_renderer_choose_video_codec(current_video_is_h265);
    }
    mux_to_file = true;
    return 1;
}

extern "C" void uxplay_stop_recording() {
    std::lock_guard<std::mutex> lock(mux_recording_mutex);
    if (!mux_to_file) return;
    mux_to_file = false;
    if (!recording_test_mode) {
        mux_renderer_stop();
...
extern "C" void video_reset(void *cls, reset_type_t type) {
    if (type != RESET_TYPE_ON_VIDEO_PLAY) {
        emit_host_event(UXPLAY_EVENT_STREAM_STOPPED, NULL, NULL, NULL,
                        "The AirPlay stream ended");
    }
    switch (type) {
...
extern "C" void display_pin(void *cls, char *pin) {
    emit_host_event(UXPLAY_EVENT_PIN_REQUIRED, NULL, NULL, NULL, pin);
    int margin = 10;
    int spacing = 3;
...
extern "C" void conn_reset (void *cls, int reason) {
    ...
    case 1:
        LOGI("*** ERROR lost connection with client (network problem?)");
	    emit_host_event(UXPLAY_EVENT_WARNING, NULL, NULL, NULL,
                            "Connection to the AirPlay device was lost");
	break;
    case 2:
        LOGI("*** ERROR Unsupported HLS streaming source: (exit attempt to stream)");
	    emit_host_event(UXPLAY_EVENT_ERROR, NULL, NULL, NULL,
                            "This HLS streaming source is not supported");
	break;      
    default:
      break;
    }
...
extern "C" void report_client_request(void *cls, char *deviceid, char * model, char *name, bool * admit) {
    ...
    emit_host_event(UXPLAY_EVENT_CLIENT_CONNECTING, name, model, deviceid,
                    "AirPlay device is connecting");
    if (restrict_clients) {
...
extern "C" void video_report_size(void *cls, float *width_source, float *height_source, float *width, float *height) {
    emit_host_event(UXPLAY_EVENT_MIRRORING_STARTED, NULL, NULL, NULL,
...
#define PID_MAX 4194304 // 2^22
    ...
    emit_host_event(UXPLAY_EVENT_ENGINE_READY, server_name.c_str(), NULL, NULL,
                    "Ready for AirPlay connections");
    reconnect:
    compression_type = 0;
