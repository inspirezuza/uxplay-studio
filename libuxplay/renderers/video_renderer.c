...
static bool attach_embedded_window(GstElement *video_sink) {
    ...
    if (!video_sink || !GST_IS_VIDEO_OVERLAY(video_sink)) {
        logger_log(logger, LOGGER_ERR,
                   "The selected video sink does not support embedding in the host window");
        return false;
    }

    GObjectClass *klass = G_OBJECT_GET_CLASS(video_sink);
    if (g_object_class_find_property(klass, "force-aspect-ratio")) {
    ...
    for (int i = 0; i < n_renderers; i++) {
        ...
        if (hls_video) {
            ...
            if (embedded_window_handle && (!rtp || jpeg_pipeline)) {
                ...
                g_string_append(sink_name, renderer_type[i]->codec);
                GstElement *embedded_sink = gst_bin_get_by_name(
